...
class CommandBalance(Command):
    ...
    def add_parser(self, sub_parser, common_parser):
        ...
        parser_balance.add_argument(
            "-e", "--end", type=int, default=-1, help="end block height, inclusive"
        )
        parser_balance.add_argument(
            "--count", type=int, default=-1, help="How many blocks to inspect"
        )
        parser_balance.add_argument(
            "--init-balance", type=int, default=0, help="Initial balance in loop"
        )
        parser_balance.add_argument(
            "--file", type=str, default="", help="file including transaction hashes"
        )
        parser_balance.add_argument(
            "--details",
            action="store_true",
            default=False,
            help="Show the result in more detail",
        )
    ...
    @classmethod
    def _print_details(
        cls, txs: List["Transaction"], tx_results: List["TransactionResult"]
    ):
