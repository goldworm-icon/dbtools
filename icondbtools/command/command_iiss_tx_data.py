...
class CommandIISSTXData(Command):
    ...
    def add_parser(self, sub_parser, common_parser):
        name = 'iisstxdata'
        desc = 'Extract the transactions which affect I-Score amount in a given block height range \n' \
               '(setDelegation, registerPRep, unregisterPRep'

        # create the parser for account
        parser_iiss_tx_data = sub_parser.add_parser(name, parents=[common_parser], help=desc)
        parser_iiss_tx_data.add_argument('--start', type=int, default=0, help='start height', required=True)
        parser_iiss_tx_data.add_argument('--count', type=int, default=0, help='the number of blocks', required=True)
    ...
    @classmethod
    def _is_succeeded_transaction(cls, reader: 'BlockDatabaseReader', transaction: dict):
        try:
            tx_result_data: dict = reader.get_transaction_result_by_hash(transaction["txHash"])
